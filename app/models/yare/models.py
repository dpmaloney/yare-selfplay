...
def value_head(y):
    ...
    y = dense(y, 32, batch_norm=True)
    y = dense(y, 64, batch_norm=True)
...
def policy_head(y, legal_actions):
    ...
    y = dense(y, 32, batch_norm=True)
    y = dense(y, 64, batch_norm=True)
    policy = dense(y, ACTIONS, batch_norm = True, activation = None, name='pi')
...
def residual(y, filters):
    ...
    y = dense(y, filters, batch_norm = True)
    y = dense(y, filters, activation = None, batch_norm = True)
    y = Add()([shortcut, y])
    y = Activation('relu')(y)
